...
def test(args):
    ...
    weights["fc_embedding.weight"]=fc_weight
    weights["fc_embedding.bias"]=fc_bias
    ...
    np.savez(root_dir + args.split + "_set_triplet_resnet_" + args.dataset,  embeddings=outputs_embedding, lebels=labels_embedding, filenames=path_imgs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_cols', nargs='?', type=int, default=224, 
                        help='Height of the input image')
    parser.add_argument('--batch_size', nargs='?', type=int, default=25, 
                        help='Batch Size')
